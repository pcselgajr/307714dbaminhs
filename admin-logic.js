...
function openSM(d){var x=d||{lrn:'',name:'',grade:'',contact:'',status:'Active',gender:''};var e=!!d;opM(e?'Edit Student':'Add Student','<div class="fg"><label>LRN</label><input id="mf1" value="'+x.lrn+'" placeholder="136789012345"></div><div class="fg"><label>Full Name</label><input id="mf2" value="'+x.name+'"></div><div class="fg-row"><div class="fg"><label>Grade &amp; Section</label><input id="mf3" value="'+x.grade+'" placeholder="Grade 7 - Rizal"></div><div class="fg"><label>Gender</label><select id="mf6"><option value="">Select Gender</option><option value="Male"'+(x.gender==="Male"?' selected':'')+'>Male</option><option value="Female"'+(x.gender==="Female"?' selected':'')+'>Female</option></select></div></div><div class="fg-row"><div class="fg"><label>Contact</label><input id="mf4" value="'+x.contact+'" placeholder="09XX XXX XXXX"></div><div class="fg"><label>Status</label><select id="mf5"><option'+(x.status==="Active"?' selected':'')+'>Active</option><option'+(x.status==="Inactive"?' selected':'')+'>Inactive</option></select></div></div><div style="display:flex;gap:10px;margin-top:18px"><button class="btn btn-p" onclick="svS('+(e?x.id:'null')+')">'+(e?'Update':'Add')+' &#10148;</button><button class="btn btn-s" onclick="clM()">Cancel</button></div>')}
...
function svS(eid){var o={lrn:document.getElementById('mf1').value,name:document.getElementById('mf2').value,grade:document.getElementById('mf3').value,contact:document.getElementById('mf4').value,status:document.getElementById('mf5').value,gender:document.getElementById('mf6')?document.getElementById('mf6').value:''};if(!o.name){toast('Enter name','er');return};if(eid){var i=S.findIndex(function(x){return x.id===eid});if(i>-1)S[i]=Object.assign(S[i],o)}else{if(o.lrn&&S.find(function(x){return x.lrn===o.lrn})){toast('Cannot add — LRN "'+o.lrn+'" is already in the Students Directory.','er');return;}o.id=getNextId(S);S.unshift(o)};saveData('students',S);clM();rS();uS();toast(eid?'Student updated!':'Student added!','su')}

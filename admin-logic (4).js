...
function printEmployeeDTR() {
  ...
  var principal = settings.principal || '';
  
  var totalUtH = 0, totalUtM = 0;
  var rows = '';
  ...
  for (var d = 1; d <= daysInMonth; d++) {
    ...
    rows += '<tr>';
    rows += '<td class="day">' + d + '</td>';
    
    if (isWeekend) {
      rows += '<td colspan="4" class="wknd">' + dayLabel + '</td>';
      rows += '<td></td><td></td>';
      ...
      var inParts = tIn.match(/(\d+):(\d+)/);
      var amIn = '', pmOut = '';
      
      if (inParts) {
        var ih = parseInt(inParts[1]);
        var im = parseInt(inParts[2]);
        amIn = (ih > 12 ? ih-12 : ih) + ':' + (im<10?'0':'') + im;
      }
      
      var utH = '', utM = '';
      if (tOut) {
        var outParts = tOut.match(/(\d+):(\d+)/);
        if (outParts) {
          var oh = parseInt(outParts[1]);
          var om = parseInt(outParts[2]);
          pmOut = (oh > 12 ? oh-12 : oh) + ':' + (om<10?'0':'') + om;
          
          var schedEndMin = endH * 60 + endM;
          var actualEndMin = oh * 60 + om;
  ...
  totalUtM = totalUtM % 60;
  
  var w = window.open('','_blank');
  w.document.write('<html><head><title>DTR - ' + empName + '</title>');
  w.document.write('<style>');
  w.document.write('*{margin:0;padding:0;box-sizing:border-box}');
  w.document.write('body{font-family:"Times New Roman",serif;font-size:11px;color:#000}');
  w.document.write('.page{width:3.5in;min-height:8.7in;margin:0 auto;padding:12px 10px;border:1px solid #000}');
  w.document.write('.cs{text-align:center;font-size:9px;margin-bottom:2px}');
  w.document.write('.title{text-align:center;font-size:16px;font-weight:700;letter-spacing:1px;margin-bottom:2px}');
  w.document.write('.name{text-align:center;font-size:12px;font-weight:700;margin-bottom:2px;text-decoration:underline}');
  w.document.write('.name-label{text-align:center;font-size:8px;font-style:italic;margin-bottom:6px}');
  w.document.write('.month-row{display:flex;justify-content:center;align-items:baseline;gap:4px;font-size:10px;margin-bottom:2px}');
  w.document.write('.month-val{font-size:14px;font-weight:700;border-bottom:1px solid #000;padding:0 10px}');
  w.document.write('.sched-row{display:flex;justify-content:space-between;font-size:9px;margin-bottom:6px;font-style:italic}');
  w.document.write('.sched-row span{border-bottom:1px dashed #000;padding:0 4px}');
  w.document.write('table{width:100%;border-collapse:collapse;margin-bottom:4px}');
  w.document.write('th{border:1px solid #000;padding:1px 2px;font-size:8px;font-weight:700;text-align:center;background:#f5f5f5}');
  w.document.write('td{border:1px solid #000;padding:0px 3px;font-size:9px;text-align:center;height:11px;line-height:11px}');
  w.document.write('.day{font-weight:700;width:20px}');
  w.document.write('.wknd{font-size:8px;font-weight:700;letter-spacing:1px;color:#333}');
  w.document.write('.total td{font-weight:700;font-size:9px;letter-spacing:4px}');
  w.document.write('.cert{font-size:9px;font-style:italic;margin:8px 0;line-height:1.5;text-indent:20px}');
  w.document.write('.sig-area{margin-top:10px;font-size:10px}');
  w.document.write('.sig-name{text-align:center;font-weight:700;font-size:11px;text-decoration:underline;margin-top:24px}');
  w.document.write('.sig-label{text-align:center;font-size:9px;font-style:italic}');
  w.document.write('.sig-pos{text-align:center;font-size:9px;font-weight:700}');
  w.document.write('.note{text-align:center;font-size:8px;font-style:italic;margin-top:4px}');
  w.document.write('@page{size:3.5in 8.7in;margin:0}');w.document.write('@media print{.page{border:none;padding:8px 6px;width:3.5in;min-height:8.7in}body{margin:0}}');
  w.document.write('</style></head><body>');
  
  w.document.write('<div class="page">');
  
  // Header
  w.document.write('<div class="cs">CIVIL SERVICE FORM No. 48</div>');
  w.document.write('<div class="title">DAILY TIME RECORD</div>');
  w.document.write('<div class="name">' + empName.toUpperCase() + '</div>');
  w.document.write('<div class="name-label">(Name)</div>');
  
  // Month & Schedule
  w.document.write('<div class="month-row">');
  w.document.write('<span>For the month of</span>');
  w.document.write('<span class="month-val">' + months[mon] + ' ' + year + '</span>');
  w.document.write('</div>');
  w.document.write('<div class="sched-row">');
  w.document.write('<span>Official hours for arrival</span>');
  w.document.write('<span>Regular Days: ' + schedStart + ' - ' + schedEnd + '</span>');
  w.document.write('</div>');
  
  // Table
  w.document.write('<table>');
  w.document.write('<tr><th rowspan="2" style="width:22px">Day</th><th colspan="2">A.M.</th><th colspan="2">P.M.</th><th colspan="2">UNDERTIME</th></tr>');
  w.document.write('<tr><th>Arrival</th><th>Depar-<br>ture</th><th>Arrival</th><th>Depar-<br>ture</th><th>Hours</th><th>Minutes</th></tr>');
  w.document.write(rows);
  w.document.write('<tr class="total"><td colspan="5" style="text-align:center;letter-spacing:6px">T O T A L</td>');
  ...
  // Employee signature
  w.document.write('<div class="sig-name">' + empName.toUpperCase() + '</div>');
  
  // Verified by
  w.document.write('<div class="sig-label" style="margin-top:12px">Verified as to the prescribed office hours:</div>');
  
  // Principal
  w.document.write('<div class="sig-name" style="margin-top:20px">' + principal.toUpperCase() + '</div>');
  w.document.write('<div class="sig-pos">' + (empPos || 'School Head') + '</div>');
  w.document.write('<div class="note">(See Instructions on back)</div>');
  
  w.document.write('</div>'); // end page
  w.document.write('</body></html>');
  w.document.close();
  setTimeout(function() { w.print(); }, 500);
}
